...
export class ScheduleController {
  ...
  @Get('tasks/:name')
  @ApiOperation({
    summary: 'Get a specific scheduled task by name from memory',
  })
  ...
  async updateTaskStatus(
    ...
    const updated = await this.scheduleService.updateTaskStatus(
      name,
  ...
  async triggerTask(@Param('taskName') taskName: string) {
    ...
    switch (taskName) {
      ...
      case 'hourlyTask':
        ...
        break;
      case 'cleanupPermissionsTask':
        this.scheduleService.handleCleanupExpiredPermissions();
        break;
      ...
      default:
        ...
        if (task.handler) {
          ...
          throw new BadRequestException(
            `Task ${taskName} cannot be manually triggered (no handler defined)`,
          );
        }
    }
    ...
    if (!updatedTask) {
      throw new NotFoundException(
        `Task ${taskName} not found after triggering`,
      );
    }
